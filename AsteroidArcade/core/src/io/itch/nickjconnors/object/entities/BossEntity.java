...
import io.itch.nickjconnors.scene.Level;
...
public class BossEntity extends Entity {
	...
	public BossEntity(Texture texture, Texture weaponTexture, Level t, ArrayList<Entity> e, Sound[] l, Player p) {
		...
		this.setPosition(new Vector2((AsteroidArcadeGame.WIDTH - this.getWidth()) / 2f, AsteroidArcadeGame.HEIGHT + this.getHeight()));
	...
	public void update(float dt) {
		...
		if (this.getVelocity().y != 0 && this.getPosition().y < AsteroidArcadeGame.HEIGHT - this.getHeight() * 2.5f) {
		...
		switch (type) {
			case BOSS_UFO:
				this.getPosition().x = AsteroidArcadeGame.WIDTH / 3f * (float) Math.cos(2 * timer) + 256 - 96f;
				// Shoot in middle of screen
				fireChance = 0f;
				if(this.getPosition().x > AsteroidArcadeGame.WIDTH / 4 && this.getPosition().x < AsteroidArcadeGame.WIDTH * 3/4)
					fireChance = 2f;
				break;
			...
			case BOSS_LASER:
				...
				if(this.getVelocity().x != 0 &&
						(this.getPosition().x > AsteroidArcadeGame.WIDTH - this.width || this.getPosition().x <= 0)) {
					this.getVelocity().x = 0;
					this.milestone = this.timer + 2f;					
				}
				
				if(timer > milestone && this.getPosition().x > AsteroidArcadeGame.WIDTH - this.width) {
					this.getVelocity().x = -150f;
				}
		...
		if (Math.random() < fireChance * dt) {
			...
			if (ents != null) {
				if (this.weapon.type != Weapon.Type.LASER)
					...
					lasers[2].play(AsteroidArcadeGame.masterVolume * 0.25f);
