...
public class OptionScene extends Scene {
	...
	private GameObject sound, music, back, link;
	...
	public OptionScene(SceneManager sm, AssetManager am, BitmapFont[] f) {		
		...
		asteroid.setPosition(new Vector2(16, AsteroidArcadeGame.HEIGHT - 128));
		...
		sound.setPosition(new Vector2(AsteroidArcadeGame.WIDTH / 2 - sound.getWidth() * 2, 400));
		this.guiObjects.add(sound);
		
		music = new GameObject().setTexture(2f, -1f, musicTexture, muteMusicTexture);
		music.setPosition(new Vector2(AsteroidArcadeGame.WIDTH/2 + music.getWidth(), 400));
		this.guiObjects.add(music);
		
		back = new GameObject().setTexture(2f, xTexture);
		back.setPosition(new Vector2(AsteroidArcadeGame.WIDTH - 40, AsteroidArcadeGame.HEIGHT - 40));
		this.guiObjects.add(back);
		
		TextObject text1 = new TextObject("OPTIONS", 2f, font[0]);
		text1.setPosition(new Vector2(0f, AsteroidArcadeGame.HEIGHT - 160));
		text1.centerX();
		this.guiObjects.add(text1);

		TextObject text2 = new TextObject("SOUND", 1f, font[0]);
		text2.setPosition(new Vector2(0f, AsteroidArcadeGame.HEIGHT - 192));
		text2.centerX();
		...
		link = new TextObject("Go to nickjconnors.itch.io", 1f, font[1]).setPosition(new Vector2(8, 8));
		this.guiObjects.add(link);
	}
	...
	public void handleInput(float dt, Vector2 mouse, float roll) {
		if(Gdx.input.justTouched()) {			
			if(sound.isTouched(mouse)) {
				sound.changeFrame();
				soundMute = !soundMute;
				AsteroidArcadeGame.masterVolume = soundMute ? 0f : 0.5f;
			...
			if(music.isTouched(mouse)) {
				...
				AsteroidArcadeGame.musicVolume = musicMute ? 0f : 0.5f;
			...
			if(link.isTouched(mouse)) {
				Gdx.net.openURI("https://nickjconnors.itch.io/");
			}
		}
	}
