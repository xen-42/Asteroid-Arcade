...
import io.itch.nickjconnors.object.entities.Attractor;
import io.itch.nickjconnors.object.entities.BossEntity;
import io.itch.nickjconnors.object.entities.Entity;
import io.itch.nickjconnors.object.entities.Particle;
import io.itch.nickjconnors.object.entities.PowerUp;
import io.itch.nickjconnors.object.entities.SwarmEntity;
import io.itch.nickjconnors.object.entities.Entity.Team;
...
public class PlayScene extends Scene {
	...
	public PlayScene(SceneManager sm, AssetManager am, BitmapFont[] f, int selection) {
		...
		pauseButton.setPosition(new Vector2(AsteroidArcadeGame.WIDTH - 40, AsteroidArcadeGame.HEIGHT - 32));
		...
		optionButton.setPosition(new Vector2(AsteroidArcadeGame.WIDTH - 40, AsteroidArcadeGame.HEIGHT - 64));

		GameObject coin = new GameObject().setTexture(1.5f, coinTexture);
		coin.setPosition(new Vector2(8, AsteroidArcadeGame.HEIGHT - 80));
		guiObjects.add(coin);
		...
		case 1:
			...
			player.setHealth(20);
			player.setWeapon(Weapon.Type.MISSILE, 6f, missileTexture);
			player.setEmitsSmoke(true);
		...
		case 5:
			...
			player.setHealth(10);
			player.setWeapon(Weapon.Type.FLAK, 5f, flakTextures);
			player.setEmitsSmoke(true);
			
			//Shield
			fgObjects.add(player.addShield(5, shieldTexture, 4.5f));
		}
		player.setPosition(new Vector2((AsteroidArcadeGame.WIDTH - player.getWidth()) / 2f, AsteroidArcadeGame.HEIGHT / 5));
		entities.add(player);
		...
		pauseText.setPosition(new Vector2(0, AsteroidArcadeGame.HEIGHT - 128));
		pauseText.centerX();
		pauseText.setVisibility(false);
		this.guiObjects.add(pauseText);
		
		levelText = new TextObject("", 1f, font[0]);
		levelText.setPosition(new Vector2(8, AsteroidArcadeGame.HEIGHT - 24));
		this.guiObjects.add(levelText);
		
		timerText = new TextObject("", 1f, font[0]);
		timerText.setPosition(new Vector2(8, AsteroidArcadeGame.HEIGHT - 48));
		this.guiObjects.add(timerText);
		
		scoreText = new TextObject("", 1.4f, font[0]);
		scoreText.setPosition(new Vector2(40, AsteroidArcadeGame.HEIGHT - 75));
	...
	public void update(float dt) {
		if (!paused) {
			...
			if (!player.isMarkedForRemoval() && player.getHealth() <= 0) {
				...
				death.play(AsteroidArcadeGame.masterVolume);
				...
				o.setPosition(new Vector2(0, AsteroidArcadeGame.HEIGHT - 288));
				o.centerX();
				guiObjects.add(o);
				
				s = "Best time: " + (player.timer > bestTime ? formatTime(player.timer) + " NEW" : formatTime(bestTime));
				o = new TextObject(s, 1f, font[0]);
				o.setPosition(new Vector2(0, AsteroidArcadeGame.HEIGHT - 336));
				o.centerX();
				guiObjects.add(o);			
			}
		}
	...
	public void updateEntity(float dt, int i) {
		...
		if (e.isMarkedForRemoval()) {
			if (!(e instanceof PowerUp)) {
				...
				pickups[(int) (Math.random() * 3)].play(AsteroidArcadeGame.masterVolume);
		}
	...
	private void newAsteroid() {
		...
		float x = (float) Math.random() * (AsteroidArcadeGame.WIDTH - asteroid.getWidth());
		float y = inverted ? -asteroid.getHeight() : AsteroidArcadeGame.HEIGHT;
	...
	private void newDebris() {
		...
		float x = (float) Math.random() * (AsteroidArcadeGame.WIDTH - satellite.getWidth());
		float y = inverted ? -satellite.getHeight() : AsteroidArcadeGame.HEIGHT;
	...
	private void newMine() {
		...
		do {
			float x = (float) Math.random() * (AsteroidArcadeGame.WIDTH - mine.getWidth());
			float y = AsteroidArcadeGame.HEIGHT;
	...
	private void newExplosion(Entity e) {
		...
		explosions[(int) (Math.random() * 3)].play(AsteroidArcadeGame.masterVolume * 0.75f);
	}
		...
		if (e != null) {
			if(!ignoreSound) {
				if (player.getWeapon().type != Weapon.Type.LASER)
					...
					lasers[1].play(AsteroidArcadeGame.masterVolume * 0.25f);
				}
			}
	...
	private void invert() {
		...
		//Invert player
		if(inverted) {
			player.getPosition().y = (AsteroidArcadeGame.HEIGHT*4/5) - player.getHeight();
		} else {
			player.getPosition().y = AsteroidArcadeGame.HEIGHT / 5;
		}
	...
	private void newWormhole() {
		...
		wormhole.setPosition(new Vector2((AsteroidArcadeGame.WIDTH - wormhole.getWidth()) / 2f, AsteroidArcadeGame.HEIGHT));
	...
	private void newPowerUp(PowerUp.Type type) {
		...
		//Same x
		float x = ((int) ((Math.random() * (AsteroidArcadeGame.WIDTH - 32)) / 32)) * 32f;
		for (int i = 0; i < number; i++) {
			float y = inverted ? ((i + 1) * -32) : AsteroidArcadeGame.HEIGHT + i * 32;
	...
	private void newStar(float scale, boolean randomY) {
		...
		float x = ((int) ((Math.random() * (AsteroidArcadeGame.WIDTH - star.getWidth())) / star.getWidth())) * (int) star.getWidth();
		
		float y;
		if(randomY)
			y = ((int) ((Math.random() * (AsteroidArcadeGame.HEIGHT - star.getWidth())) / star.getWidth())) * (int) star.getWidth();
		else
			y = (inverted ? -star.getHeight() : AsteroidArcadeGame.HEIGHT);
